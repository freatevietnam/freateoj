...
class Organization(models.Model):
    ...
    monthly_free_credit_limit = models.FloatField(
        default=settings.FREATEOJ_MONTHLY_FREE_CREDIT,
        help_text=_('Amount of free credits allocated each month'),
    )
    _pp_table = [pow(settings.FREATEOJ_ORG_PP_STEP, i) for i in range(settings.FREATEOJ_ORG_PP_ENTRIES)]
    ...
    def calculate_points(self, table=_pp_table):
        ...
        pp = settings.FREATEOJ_ORG_PP_SCALE * sum(ratio * pp for ratio, pp in zip(table, data))
    ...
    def max_problems(self):
        return settings.FREATEOJ_ORGANIZATION_DEFAULT_MAX_PROBLEMS + (self._active_quota_agg['total_problems'] or 0)

    @cached_property
    def max_storage(self):
        return settings.FREATEOJ_ORGANIZATION_DEFAULT_MAX_STORAGE + (self._active_quota_agg['total_storage'] or 0)
...
class Profile(models.Model):
    ...
    display_rank = models.CharField(max_length=10, default='user', verbose_name=_('display rank'),
                                    choices=settings.FREATEOJ_DISPLAY_RANKS)
    ...
    def has_enough_solves(self):
        return self.problem_count >= settings.FREATEOJ_INTERACT_MIN_PROBLEM_COUNT
    ...
    def can_tag_problems(self):
        if self.allow_tagging:
            if self.user.has_perm('judge.add_tagproblem'):
                return True
            if self.rating is not None and self.rating >= settings.FREATEOJ_TAG_PROBLEM_MIN_RATING:
                return True
        return False
    ...
    def calculate_contribution_points(self):
        ...
        new_pp = (total_comment_scores + total_blog_scores) * settings.FREATEOJ_CP_COMMENT + \
            count_good_tickets * settings.FREATEOJ_CP_TICKET + \
            count_suggested_problem * settings.FREATEOJ_CP_PROBLEM
        if new_pp != old_pp:
