...
class CreateOrganization(PermissionRequiredMixin, TitleMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if self.has_permission():
            if self.request.user.profile.admin_of.count() >= settings.FREATEOJ_ORGANIZATION_ADMIN_LIMIT and \
               not self.request.user.has_perm('spam_organization'):
                return render(request, 'organization/create-limit-error.html', {
                    ...
                    'admin_limit': settings.FREATEOJ_ORGANIZATION_ADMIN_LIMIT,
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            OrganizationQuota.objects.create(
                ...
                added_problems=packages * settings.FREATEOJ_QUOTA_PACKAGE_PROBLEMS,
                added_storage=packages * settings.FREATEOJ_QUOTA_PACKAGE_STORAGE,
            )
...
class ProblemCreateOrganization(AdminOrganizationMixin, ProblemCreate):
    ...
    def _quota_error_response(self):
        return render(self.request, 'organization/quota-error.html', {
            ...
            'quota_warning_suffix': settings.FREATEOJ_QUOTA_WARNING_SUFFIX,
        })
    ...
    def get(self, request, *args, **kwargs):
        if settings.FREATEOJ_QUOTA_ENFORCEMENT_ENABLED and not self.organization.can_create_problem():
...
class OrganizationStorageDashboard(LoginRequiredMixin, TitleMixin, AdminOrganizationMixin,
    ...
    def get_context_data(self, **kwargs):
        ...
        context['cost_chart'] = get_lines_chart(days, {
            _('Cost (thousand vnd)'): [
                round(max(0, c - settings.FREATEOJ_MONTHLY_FREE_CREDIT) / sec_per_hour * settings.FREATEOJ_PRICE_PER_HOUR, 3)
