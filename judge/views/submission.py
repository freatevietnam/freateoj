...
class SubmissionsListBase(DiggPaginatorMixin, TitleMixin, ListView):
    ...
    def _get_queryset(self):
        ...
        if self.selected_languages:
            # Force eager evaluation to get the IDs for efficient filtering.
    ...
    def is_in_low_power_mode(self):
        return settings.FREATEOJ_LOW_POWER_MODE and not self.request.user.is_superuser
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.is_in_low_power_mode():
            max_page = settings.FREATEOJ_LOW_POWER_MODE_CONFIG.get('max_page', 5)
