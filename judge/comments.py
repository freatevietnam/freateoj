...
class CommentForm(ModelForm):
    ...
    def clean_body(self):
        ...
        if self.request is not None and not self.request.user.is_staff:
            if len(body) < settings.FREATEOJ_COMMENT_MIN_LENGTH:
                raise ValidationError(_('Comment is too short (min %d chars).') % settings.FREATEOJ_COMMENT_MIN_LENGTH)
            if len(body) > settings.FREATEOJ_COMMENT_MAX_LENGTH:
                raise ValidationError(_('Comment is too long (max %d chars).') % settings.FREATEOJ_COMMENT_MAX_LENGTH)

            if settings.FREATEOJ_COMMENT_BLACKLIST_TERMS:
                body_casefolded = body.casefold()
                blacklist_casefolded = [term.casefold() for term in settings.FREATEOJ_COMMENT_BLACKLIST_TERMS]
        ...
        if self.request is not None and self.request.user.is_authenticated:
            ...
            if profile.mute:
                ...
                raise ValidationError(_('You need to have solved at least %d problems '
                                        'before your voice can be heard.') % settings.FREATEOJ_INTERACT_MIN_PROBLEM_COUNT)
            if not self.request.user.is_staff:
                if profile.contribution_points < settings.FREATEOJ_COMMENT_MIN_CONTRIBUTION:
                    raise ValidationError(
                        _('You need at least %d contribution points to comment.')
                        % settings.FREATEOJ_COMMENT_MIN_CONTRIBUTION,
                    )

                if (settings.FREATEOJ_COMMENT_RATE_LIMIT_COUNT is not None and
                        Comment.objects.filter(
                            author=profile,
                            time__gte=timezone.now() - settings.FREATEOJ_COMMENT_RATE_LIMIT_WINDOW,
                        ).count() >= settings.FREATEOJ_COMMENT_RATE_LIMIT_COUNT):
