...
UPDATE judge_comment SET revisions = versions.count
FROM (
    SELECT CAST(reversion_version.object_id AS INTEGER) AS id, COUNT(*) AS count
    FROM reversion_version
    WHERE reversion_version.content_type_id = %s AND
          reversion_version.db = %s
    GROUP BY 1
) versions WHERE judge_comment.id = versions.id;
