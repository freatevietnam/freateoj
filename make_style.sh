...
build_style() {
  ...
  npx sass resources:sass_processed --no-source-map
  npx postcss \
      ...
      --verbose --use autoprefixer --no-map -d "$2"
...
npx postcss resources/notification.css --verbose --use autoprefixer --no-map -o resources/notification.css
