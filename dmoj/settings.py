...
FREATEOJ_OFFICIAL_CONTEST_MODE = False
...
FREATEOJ_HOMEPAGE_TOP_USERS_COUNT = 5

FREATEOJ_DISPLAY_RANKS = (
    ('user', _('Normal User')),
...
FREATEOJ_PROBLEM_TIMELIMIT_LIMIT = 5
# Maximum contest duration (day) that a user can set for a contest
# without the `long_contest_duration` permission
FREATEOJ_CONTEST_DURATION_LIMIT = 14
# Maximum number of test cases that a user can create for a problem
# without the `create_mass_testcases` permission
FREATEOJ_TESTCASE_HARD_LIMIT = 100
# If a user without the `create_mass_testcases` permission create more than this amount of test
# they will receive a warning
FREATEOJ_TESTCASE_SOFT_LIMIT = 50
# Minimum problem count required to interact (comment, vote and update profile)
FREATEOJ_INTERACT_MIN_PROBLEM_COUNT = 5
# Minimum problem count required to create new blogs
FREATEOJ_BLOG_MIN_PROBLEM_COUNT = 10

# Comment validation settings
FREATEOJ_COMMENT_MIN_CONTRIBUTION = -20
FREATEOJ_COMMENT_MIN_LENGTH = 10
FREATEOJ_COMMENT_MAX_LENGTH = 10000
FREATEOJ_COMMENT_BLACKLIST_TERMS = []
FREATEOJ_COMMENT_RATE_LIMIT_COUNT = None  # maximum number of comments allowed within the time window
FREATEOJ_COMMENT_RATE_LIMIT_WINDOW = datetime.timedelta(seconds=600)

FREATEOJ_TESTCASE_VISIBLE_LENGTH = 60

FREATEOJ_TAG_PROBLEM_MIN_RATING = 1900  # Minimum rating to be able to tag a problem

FREATEOJ_SHOULD_BAN_FOR_CHEATING_IN_CONTESTS = False
FREATEOJ_CONTEST_CHEATING_BAN_MESSAGE = 'Banned for multiple cheating offenses during contests'
FREATEOJ_MAX_DISQUALIFICATIONS_BEFORE_BANNING = 3
...
FREATEOJ_BAN_COUNT_FROM_DATE = datetime.datetime(2026, 1, 1, tzinfo=datetime.timezone.utc)

# List of subdomain that will be ignored in organization subdomain middleware
FREATEOJ_IGNORED_ORGANIZATION_SUBDOMAINS = ['oj', 'www', 'localhost']

# Enable organization credit system, if true, org will not be able to submit submissions
# if they run out of credit
FREATEOJ_ENABLE_ORGANIZATION_CREDIT_LIMITATION = False
# 3 hours free per month
FREATEOJ_MONTHLY_FREE_CREDIT = 3 * 60 * 60
FREATEOJ_PRICE_PER_HOUR = 50

# Organization quota limits
FREATEOJ_ORGANIZATION_DEFAULT_MAX_PROBLEMS = 1000
FREATEOJ_ORGANIZATION_DEFAULT_MAX_STORAGE = 5 * 1024 * 1024 * 1024  # 5GB
...
FREATEOJ_QUOTA_WARNING_SUFFIX = ''
FREATEOJ_QUOTA_WARNING_THRESHOLD = 0.8
...
FREATEOJ_QUOTA_ENFORCEMENT_ENABLED = False
FREATEOJ_QUOTA_PACKAGE_STORAGE = 5 * 1024 * 1024 * 1024  # 5 GB per package
FREATEOJ_QUOTA_PACKAGE_PROBLEMS = 1000  # problems per package
FREATEOJ_LONG_QUEUE_ALERT_THRESHOLD = 10

# Low power mode: Optimize queries by limiting data scope for performance
FREATEOJ_LOW_POWER_MODE = False
FREATEOJ_LOW_POWER_MODE_CONFIG = {
...
FREATEOJ_MAGAZINE_TAG_SLUG = None
...
OJ_PROBLEM_PRESET = [
    ...
    {
        'regex': r'^https://oj\.freate\.io\.vn/problem/(?P<codename>\w+)$',
        'codename': 'FREATEOJ_%s',
        'judge': 'FreateOJ',
    },
    {
...
SITE_FULL_URL = None  # ie 'https://oj.freate.io.vn', please remove the last / if needed

ACE_URL = '/static/freateoj/ace/1.4.14'
SELECT2_JS_URL = '/static/freateoj/select2/4.0.3/js/select2.min.js'
SELECT2_CSS_URL = '/static/freateoj/select2/4.0.3/css/select2.min.css'
...
FREATEOJ_PROBLEM_DELETION_GRACE_PERIOD = datetime.timedelta(days=7)
# Maximum time the garbage collection task is allowed to run per invocation
FREATEOJ_PROBLEM_GARBAGE_COLLECTOR_TIME_LIMIT = datetime.timedelta(hours=1)
FREATEOJ_PROBLEM_GARBAGE_COLLECTOR_CRONTAB_KWARGS = {'minute': 0, 'hour': 0}
...
JQUERY_JS = '/static/freateoj/jquery/3.4.1/jquery.min.js'
FONTAWESOME_CSS = '/static/freateoj/font-awesome/4.3.0/css/font-awesome.min.css'
DMOJ_CANONICAL = 'oj.freate.io.vn'
